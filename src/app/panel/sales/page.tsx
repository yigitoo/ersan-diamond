...
export default function SalesPage() {
  ...
  const PAYMENT_OPTIONS = [
    { value: "CASH", label: t("Nakit", "Cash") },
    { value: "TRANSFER", label: t("Banka Transferi", "Bank Transfer") },
    { value: "CARD", label: t("Kredi Kartı", "Credit Card") },
    { value: "CRYPTO", label: t("Kripto", "Crypto") },
  ...
  const validateForm = () => {
    ...
    if (!form.buyerName.trim()) errors.buyerName = t("Alıcı adı gerekli", "Buyer name is required");
    if (!form.salePrice || isNaN(Number(form.salePrice)) || Number(form.salePrice) <= 0) errors.salePrice = t("Geçerli bir fiyat gerekli", "Valid price is required");
    if (!form.soldAt) errors.soldAt = t("Tarih gerekli", "Date is required");
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        alert(err.error || t("Satış kaydedilemedi", "Failed to record sale"));
        return;
      }
      refreshData();
      setDialogOpen(false);
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Record Sale Dialog */}
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} title={t("Satış Kaydet", "Record Sale")} className="max-w-xl">
        <div className="space-y-4">
          ...
          <Input label={t("Satış Tarihi", "Sale Date")} type="datetime-local" value={form.soldAt} onChange={(e) => updateForm("soldAt", e.target.value)} error={formErrors.soldAt} />

          <Textarea label={t("Notlar", "Notes")} rows={3} value={form.notes} onChange={(e) => updateForm("notes", e.target.value)} placeholder={t("İsteğe bağlı notlar...", "Optional notes...")} />
      ...
      {/* Sale Detail Sheet */}
      <Sheet open={sheetOpen} onClose={() => { setSheetOpen(false); setSelectedSale(null); }} title={t("Satış Detayları", "Sale Details")}>
        {selectedSale ? (
          <div className="space-y-6">
            <div className="space-y-3">
              <h3 className="text-xs font-medium tracking-wider uppercase text-mist">{t("Ürün", "Product")}</h3>
            ...
            <div className="space-y-3">
              <h3 className="text-xs font-medium tracking-wider uppercase text-mist">{t("Alıcı", "Buyer")}</h3>
              <div className="space-y-1">
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                ...
                <div>
                  <span className="text-mist">{t("Satış Temsilcisi", "Sales Rep")}</span>
